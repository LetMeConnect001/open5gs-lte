...
int ogs_dict_s13_entry(char *conffile)
{
    ...
    /* Command section.
     *
     * Note: unlike other dict.c files in this tree, this file does not
     * also declare an "AVP section" for Terminal-Information and its
     * children (IMEI / 3GPP2-MEID / Software-Version), nor for
     * Equipment-Status. Those AVPs are already registered by the
     * dict_dcca_3gpp freeDiameter extension (loaded via
     * configs/freeDiameter/mme.conf.in) and, for Terminal-Information's
     * sub-rules, by lib/diameter/s6a/dict.c, which the MME always
     * initializes before this module. Redeclaring them here would make
     * fd_dict_new(DICT_RULE, ...) return EEXIST, which PARSE_loc_rules
     * turns into EINVAL, aborting MME startup. */
    {
